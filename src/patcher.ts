...
export interface IPatcher {
  apply(content: string, patchSyntax: fjp.Operation[]): Promise<string>;
}
...
export async function globFilesAsync(
  patterns: string,
...
export async function patchAsync(
  ...
  for await (const file of globber.globGenerator()) {
    ...
    try {
      fileContent.content = await patcher.apply(fileContent.content, patches);
...
export function parsePatchSyntax(patchSyntax: string): Operation[] {
  ...
  XRegExp.forEach(patchSyntax, XRegExp(regex, "gm"), (match: MatchArray) => {
    if (!match.groups) {
      throw new Error(
        `Unable to parse patch syntax at line ${match.index}: '${match.input}'`
      );
    }
